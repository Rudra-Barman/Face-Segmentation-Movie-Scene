...
def load_unet():
    if not os.path.exists(MODEL_PATH):
        ...
        st.info("Make sure 'unet_face_segmentation.keras' is inside the 'models' folder")
        return None
...
def recognize_celebrities(img_rgb, mask):
    ...
    for lid in range(1, n_labels):
        ...
        # ── Celebrity name matching (FIXED) ──────────────────
        try:
            tmp = os.path.join(BASE_DIR, 'tmp_face.jpg')
            # Crop ko thoda bada karo aur save karo
                        ...
                        try:
                            ...
                            if len(df_res) > 0 and len(df_res[0]) > 0:
                                ...
                                if dist < distance_val:
                                    ...
                                    # File name se actor naam nikalo
                                    fname = os.path.splitext(
                                        os.path.basename(best_path)
                                    )[0]
                                    # underscore ko space mein badlo
                                    actor_name = fname.replace('_', ' ')
                            if distance_val < 0.6:
                                break  # Good match mila, stop
                        except Exception:
                            continue

                    # Agar distance zyada hai to Unknown rakho
                    if distance_val >= 0.6:
                        actor_name = 'Unknown'
...
with st.sidebar:
    ...
    """)
    st.markdown('---')
    st.markdown('**💡 Tip:** Add more photos of an actor in `celebrity_faces` folder for better accuracy!')
...
with tab1:
    ...
    if uploaded:
        ...
        if show_celeb and faces > 0:
            ...
            if not DEEPFACE_AVAILABLE:
                st.warning('DeepFace not installed. Run: pip install deepface tf-keras')
            else:
                with st.spinner('🔍 Identifying actors... (first run ~30-60s for index building)'):
